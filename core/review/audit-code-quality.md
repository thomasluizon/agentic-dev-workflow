...
> **Config inputs:** `config.repos`, `config.review.rubricPath`, `config.parity.enabled`, `config.contract.enabled`, `config.auditAnchors.scale`, `config.execution.maxParallelSubagents`, `config.execution.hasNamedAgentRegistry`
