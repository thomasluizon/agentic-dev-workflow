#!/usr/bin/env node
// Deterministic cross-model second-opinion helper. Shells the local `opencode`
// CLI to have a DIFFERENT model judge one self-contained finding, parses its
// JSONL event stream, and prints one line of JSON — always exiting 0, degrading
// to UNAVAILABLE on any failure (absent binary / error event / timeout /
// unparseable / unfunded). Prompt is read from stdin so diffs are never subject
// to argv length or quoting limits. Runtime-agnostic: no project strings.
