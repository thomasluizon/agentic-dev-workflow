...
> **Config inputs:** `config.pr.baseBranch`, `config.pr.squash`, `config.pr.commitTrailer`, `config.pr.prBodyFooter`, `config.branchNaming`
...
4. **Commit all changes:**
   ...
   - Append `{{config.pr.commitTrailer}}` to the message if it is set.
...
```bash
gh pr create --base {{config.pr.baseBranch}} --head <branch-name> --fill
```
   If `{{config.pr.prBodyFooter}}` is set, append it to the PR body (use `--body` instead of `--fill`, keeping the generated summary).
