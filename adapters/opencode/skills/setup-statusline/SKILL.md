---
description: Install the portable Claude Code status line (model@effort, context-usage bar, git branch, 5h + weekly rate-limit usage, logged-in account) into ~/.claude and wire it in settings.json — reproduces the same status line on any machine.
---
# setup-statusline

Install the portable Claude Code status line (model@effort, context-usage bar, git branch, 5h + weekly rate-limit usage, logged-in account) into ~/.claude and wire it in settings.json — reproduces the same status line on any machine.
...
> **Core body:** `../_core/meta/setup-statusline.md`
